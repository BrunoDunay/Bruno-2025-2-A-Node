...
async function obtenerMunicipios(req, res) {
  const municipios = await Municipio.find().populate({
    path: 'estadoId',
    select: 'nombre'
  }).sort({ nombre: 1 });
  res.json(municipios);
...
async function obtenerMunicipio(req, res) {
  const municipio = await Municipio.findById(req.params.id).populate({
    path: 'estadoId',
    select: 'nombre'
  });
  if (!municipio) {
...
async function obtenerMunicipiosEstado(req, res) {
  ...
  const municipios = await Municipio.find({ estadoId }).populate({
    path: 'estadoId',
    select: 'nombre'
  }).sort({ nombre: 1 });
  res.json(municipios);
